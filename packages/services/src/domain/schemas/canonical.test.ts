...
import { BookSchema, ProjectBriefSchema, StoryBlueprintSchema, WorldFoundationSchema } from './canonical';

describe('canonical artifact schemas', () => {
  test('ProjectBriefSchema accepts the bootstrap canonical contract', () => {
    expect(
  ...
  test('WorldFoundationSchema requires the project brief ref', () => {
    expect(
  ...
  test('StoryBlueprintSchema accepts the bootstrap canonical contract', () => {
    expect(
  ...
  test('BookSchema requires global promises and a default chapter type policy', () => {
    expect(
      BookSchema.parse({
        id: 'book-001',
        title: '潮汐锁',
        ...
        globalPromises: ['p-1'],
        globalConstraints: [],
        defaultChapterTypePolicy: { maxConsecutiveSamePrimaryType: 2 },
      }),
    ).toMatchObject({ title: '潮汐锁' });
  });
});
