...
export function ArtifactDetail({ artifact, onAction, pending = false, actionForm }: ArtifactDetailProps) {
  ...
  const actionButtons = (actions: readonly ApprovalAction[], isForm: boolean) =>
    actions.map((action) => {
      ...
      return isForm ? (
        <button key={action} type="submit" name="intent" value={action} disabled={pending} style={style}>
          {ACTION_LABELS[action]}
        </button>
      ) : (
        <button key={action} type="button" data-action={action} disabled={pending} onClick={() => onAction(action, note)} style={style}>
          {ACTION_LABELS[action]}
        </button>
      );
    });
